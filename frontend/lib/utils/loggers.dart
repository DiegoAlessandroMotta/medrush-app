...
class ConsoleColor {
  ...
  static const String grayCyan = '$dim$brightCyan'; // Celeste grisáceo
  ...
  /// Formatea tiempos de respuesta: si > 1000ms muestra en segundos con 2 decimales
  static String formatResponseTime(int milliseconds) {
    if (milliseconds >= 1000) {
      final seconds = StatusHelpers.formatearNumero(milliseconds / 1000);
      return colorize('[$seconds s]', cache);
    }
    return colorize('[$milliseconds ms]', cache);
  }
}

/// Log formateado de paginación con etiqueta coloreada
void logPagination(Map<String, dynamic> pagination,
    [Map<String, dynamic>? queryParams]) {
  if (!kDebugMode) {
    return;
  }
  final label = ConsoleColor.colorize('[pagination]', ConsoleColor.grayCyan);
  final total = pagination['total'];
  final perPage = pagination['per_page'] ?? pagination['perPage'];
  final currentPage = pagination['current_page'] ?? pagination['currentPage'];
  final lastPage = pagination['last_page'] ?? pagination['lastPage'];

  // Obtener la página solicitada desde los query parameters
  final requestedPage = queryParams?['page'] ?? currentPage;

  final parts = <String>[
    if (total != null) 'total: $total',
    if (perPage != null) 'per_page: $perPage',
    if (requestedPage != null) 'requested_page: $requestedPage',
    if (currentPage != null) 'backend_current: $currentPage',
    if (lastPage != null) 'last: $lastPage',
  ];

  Logger.debug('$label ${parts.join(', ')}');
}
...
class Logger {
  ...
  static void _log(String level, String message, String color) {
    ...
    final String timestamp = StatusHelpers.formatearTiempoSegundos(
